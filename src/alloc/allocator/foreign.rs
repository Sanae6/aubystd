...
use super::{AllocateError, calculate_layout_for_dst};
...
impl<C: CStyleAllocator> Allocator for ForeignAllocator<C> {
    ...
    async fn reserve_sized<'allocator, S: Strategy, T: 'allocator>(
        &'allocator self,
    ) -> Result<S::UninitSizedHandle<'allocator, T>, AllocateError<()>> {
        let layout = Layout::new::<S::SizedData<'allocator, T>>();

        let data_ptr = self.allocator.alloc(layout).map_err(|_| ())?;
    ...
    async fn reserve_unsized<'allocator, S: Strategy>(
        &'allocator self,
        layout: Layout,
    ) -> Result<S::UninitSliceHandle<'allocator, T>, AllocateError<!>> {
        let layout = calculate_layout_for_dst::<S::SliceData<'allocator, UnsizedMaybeUninit<T>>>(
            element_count,
        )?;

        let data_ptr = self.allocator.alloc(layout)?;
        let data_ptr: NonNull<S::SliceData<'allocator, UnsizedMaybeUninit<T>>> = unsafe {
            NonNull::new_unchecked(ptr::from_raw_parts_mut(
                data_ptr.as_ptr() as *mut (),
                element_count,
            ))
        };
...
pub mod tests {
    ...
    use crate::alloc::{
        allocator::{AllocateError, ForeignAllocator, StdAlloc},

...
fn main() -> ! {
    pollster::block_on(async move {
        ...
        let handle: Unique<u32> = allocator.take_item(UNIQUE, 3).await.unwrap();
        let handle: Unique<dyn Debug> = handle;
        ...
        let mut handle: Unique<UnsizedMaybeUninit<A<[u8]>>> =
            allocator.reserve_dst(UNIQUE, 5).await.unwrap();
